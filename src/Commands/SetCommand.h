...
public:
    ...
    RespObject getKey() const { return m_cmd_args[1]; }
    RespObject getValue() const { return m_cmd_args[2]; }
    std::optional<RespObject> getOption() const
    {
        return m_cmd_args.size() > 3 ? std::optional<RespObject>(m_cmd_args[3]) : std::nullopt;
    }
    std::optional<RespObject> getExpiry() const
    {
